...
사용자의 서비스 이용 여정은 접속 후 직관적으로 대시보드를 탐색할 수 있도록 단순하게 설계되었습니다.

```mermaid
graph TD
    A["접속: 메인 대시보드"] --> B["사용자 탐색 및 참여"]
    
    B -->|카테고리 탭 클릭| C["버블 군집 재정렬 - 물리 애니메이션"]
    B -->|검색창 키워드 입력| D["해당 버블 하이라이트 및 상세 카드 노출"]
    B -->|버블 직접 클릭| E["상세 정보 팝업 카드"]
    B -->|라이벌 배틀 투표| F["트렌드 VS 배틀 투표 및 비율 차트 확인"]
    
    F -->|투표 결과 반영| G["실시간 트렌드 지수 상승 및 버블 크기 확대"]
    E -->|추천 클릭| G
    E -->|위시리스트 저장| H["나만의 위시리스트 보관 - 로컬 저장"]
    E -->|보러가기 클릭| I["Y2K 이동 브릿지 페이지 - 1초 대기"]
    I -->|자동 이동| J["외부 커머스 및 정보 상세 사이트 새 탭"]
```
